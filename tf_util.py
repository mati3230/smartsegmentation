...
def conv_max_layer(input, n_feature_maps, filter_size, conv_stride, max_pool_size, max_pool_stride, conv_padding, max_pool_padding, debugging, nr, bn_decay=None, is_training=tf.constant(True, dtype=tf.bool)):
  with tf.name_scope("conv" + nr):
    net = conv2d(input, n_feature_maps, filter_size,
                         padding=conv_padding, stride=conv_stride,
                         bn=False, is_training=is_training,
                         scope="conv" + nr, bn_decay=bn_decay, use_xavier=False)
    net = max_pool2d(net, max_pool_size, stride=max_pool_stride, padding=max_pool_padding, scope="maxpool" + nr)
    return net
...
def squeeze_tensor(t):
  ...
  i = 0
  while i < len(t_shape):
    if t_shape[i].value == 1:
      t = tf.squeeze(t, axis=i)
      t_shape = t.get_shape()
      i=-1
    i += 1
  return t
...
def pairwise_distance(point_cloud):
  ...
  point_cloud = squeeze_tensor(point_cloud)
...
def get_edge_times_feature(point_cloud, nn_idx, k=20):
  ...
  idx_ = tf.range(0, batch_size) * num_points
  idx_ = tf.reshape(idx_, [batch_size, 1, 1]) 
...
def spiderConv(feat,
  ...
  """
  with tf.variable_scope(scope) as sc:
      grouped_points_shape = grouped_points.get_shape()
      batch_size = tf.shape(grouped_points)[0]
      num_point = grouped_points_shape[1].value
      K_knn = grouped_points_shape[2].value
      in_channels = grouped_points_shape[3].value
